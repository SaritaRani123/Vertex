...
import type { SemesterResponse, CourseResponse, TermResponse } from "@/lib/api-types";
...
export default function EditTermPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        const [termRes, semRes, courseRes] = await Promise.all([
          fetch(`/api/terms/${termId}`),
          fetch("/api/semesters"),
          fetch("/api/courses"),
        ]);

        if (!termRes.ok || !semRes.ok || !courseRes.ok) {
        ...
        const semesterData: SemesterResponse[] = semJson.data ?? [];
        setSemesters(semesterData);
        setCourses(courseJson.data ?? []);

        const term: TermResponse = termJson;
        const selectedSemester = semesterData.find((s: SemesterResponse) => s.id === term.semester_id);
        setSemesterYear(selectedSemester ? String(selectedSemester.year) : "");
        setSemesterType(selectedSemester ? selectedSemester.type : "");
        setCourseId(String(term.course_id));
      } catch (e) {
        setError(e instanceof Error ? e.message : "Unable to load term");
      } finally {
        setLoading(false);
      }
    }
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      <div className="flex items-center gap-2">
        ...
        <div>
          <h1 className="text-2xl font-bold">Edit Term Assignment</h1>
          <p className="text-muted-foreground">Update semester/course assignment</p>
        </div>
      </div>
